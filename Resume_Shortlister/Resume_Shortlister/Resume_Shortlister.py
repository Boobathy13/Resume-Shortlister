...
def display_results(df, job_desc_weights=None):
    ...
    simple_table.field_names = ["Rank", "Candidate", "Total Score"]
    ...
    if job_desc_weights:
        ...
        if not matched_df.empty:
            ...
            simple_matched_table.field_names = ["Rank", "Candidate", "Total Score"]
    ...
    for i, (_, row) in enumerate(df.head(5).iterrows(), 1):
        row_data = [
            ...
            row['Candidate'],
            f"{row['Total Score']:.1f}",
            f"{row['Experience Score']:.1f}",
            f"{row['Skill Score']:.1f}",
            f"{row['Certification Score']:.1f}",
            f"{row['Project Score']:.1f}",
            f"{row['Role Score']:.1f}"
        ]
...
def main():
    ...
    # Get prioritized skills (existing functionality)
    ...
    if not results_df.empty:
        enhanced_display_results(results_df, job_description)

        # Visualization (existing functionality)
        top_candidates = results_df.sort_values('Combined_Score', ascending=False).head(5)
        if not top_candidates.empty:
            skills_data = pd.DataFrame(top_candidates['Skills'].tolist(),
                                       index=top_candidates['Candidate'])
            skills_data = skills_data.fillna(0).astype(int)

            plt.figure(figsize=(12, 6))
            skills_data.plot(kind='barh', stacked=True, colormap='viridis')
            plt.title('Skill Distribution - Top Candidates')
            plt.xlabel('Count')
